...
int employee_sortByName(void* empleadoA, void* empleadoB)
{
    ...
    retorno = strcmp((*empleado_uno).nombre, (*empleado_dos).nombre);
    if(retorno<0)
    {
	retorno = 1;
    }
...
int employee_sortById(void* empleadoA, void* empleadoB)
{
    ...
    if( (*empleado_uno).id < (*empleado_dos).id )
    {
	retorno = 1;
    }
    if( (*empleado_uno).id > (*empleado_dos).id )
...
int employee_sortBySueldo(void* empleadoA, void* empleadoB)
{
    ...
    if( (*empleado_uno).sueldo < (*empleado_dos).sueldo )
    {
	    retorno = 1;
    }
    if( (*empleado_uno).sueldo > (*empleado_dos).sueldo)
...
int employee_edit(Employee* this)
{
	...
	{
	    case 1:
		if(getChar(aux_name,sizeof(aux_name) ,"Ingrese nuevo nombre :","Error", 2)==0)
		{
	    ...
	    case 3:
		employee_showEmployee(this);
		if(getfloat(&aux_salary,"\nIngrese nuevo salario [$1,000 - $10,000]","Error",min_salary,max_salary,2)==0)
		{
